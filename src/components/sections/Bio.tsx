...
function RetratoPlaceholder({ label, ratio = "aspect-[4/5]" }: { label: string; ratio?: string }) {
  return (
    <div
      role="img"
      aria-label={label}
      className={`${ratio} flex items-end overflow-hidden rounded-2xl border border-border bg-surface p-4 text-xs uppercase tracking-[0.08em] text-muted-foreground`}
    >
      {label}
    </div>
  );
