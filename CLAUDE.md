...
- 2026-09-24 — **A CHAMADA EM ROLAGEM VOLTOU PARA A GRADE DE TRÊS COLUNAS DO TEMPLATE**, e é REVERSÃO PEDIDA por ele no dia seguinte, com o print do original na mão: "não gostei dos centralizados ali, faça igual ao template... aumente ali e fique para o centro o celular, e a mensagem fique à esquerda". O agrupamento centrado (celular + textos + botão juntos) tinha sido pedido por ele na véspera. **Não "corrigir" de volta sem falar com ele.** A terceira coluna fica VAZIA: no template ela carrega o wordmark gigante da marca, e ele pediu duas vezes que o "SOBERS" não existisse. É ela que põe o aparelho no centro exato do cartão.
- 2026-09-24 — ⚠️ **O APARELHO É CORTADO PELO PÉ DO CARTÃO, de propósito** ("não completando o celular, igual tá cortado ali"). A conta não tem número mágico: ele é ANCORADO NO TOPO da própria célula e tem altura MAIOR que ela, então quem decide onde ele acaba é a borda do cartão, que já é `overflow-hidden`. Em qualquer janela ele sangra sozinho. Medido: corte de 92px em 1920x1000, 172 em 1440x900, 185 em 1440x800, 183 em 390x844.
- 2026-09-24 — ⚠️ **O RECUO DO APARELHO É PERCENTUAL DA CÉLULA E NÃO PX**, e é o que o mantém fora da pílula de navegação — ela é FIXA e termina em 98px, e um número em px não sabe a altura da janela. Foi o defeito que ele reportou com print. Primeira tentativa a 9% deu folga de 5px em 1440x900 e **-6px em 1440x800**, ou seja o aparelho por trás do menu; a 14% a folga vira 47/31px, e o pior caso medido é 17px em 1280x720.
- 2026-09-24 — **O APARELHO É O DO TEMPLATE, e o tamanho dele é UMA CONTA**: o elemento recebe `font-size: calc(<altura> / 580)` e TODA medida interna é escrita em `em` com o número que o template usa em px — moldura 280x580, ilha 100x28, os quatro botões físicos, raios de 48 e 40, e o texto dos balões. Assim a proporção é idêntica ao original em qualquer altura de janela. Cravar px faria a ilha e os balões encolherem em relação ao corpo quando o aparelho crescesse, que é o defeito clássico de mockup redimensionado. Medido: proporção 0,483 (280/580 = 0,4828) em repouso, nas seis larguras.
- 2026-09-24 — **`PistaDeRolagem` virou COMPONENTE**, a pedido ("o sinal da scroll também precisa ser o mesmo"): a tela de entrada e a chamada em rolagem usam o MESMO elemento, e quem posiciona e apaga é cada seção. Duas cópias do mesmo gesto divergem na primeira correção — foi o argumento que fez o `AvisoCasos` virar componente.
- 2026-09-24 — ⚠️ **A pista da chamada nasceu DENTRO do cartão e saiu de lá no mesmo dia, por causa do render:** com o aparelho centrado e sangrando pelo pé, não existe lugar embaixo que não seja o celular — ela caía em cima dele. Foi para junto da MANCHETE, em `z-10`, onde some com ela e é coberta pelo cartão que sobe sem precisar de mais nenhuma conta. É a mesma curva do hero: "ao scrollar, essa informação some".
- 2026-09-24 — ⚠️ **Com o texto EM CIMA no celular, a manchete passava POR BAIXO do botão do menu.** A ordem nova (texto → aparelho) faz a primeira linha nascer em 47px numa janela de 390, e a pílula termina em 74px. Medido em 390, 320 e 768: cruzava nas três. `pt-20 md:pt-24` resolve, com folga de 31px no celular e 17px em 768; de `lg` para cima o texto está na coluna da esquerda, longe da pílula centralizada, e o `pt` volta a zero.
- 2026-09-24 — ⚠️ **13º falso positivo de medição da memória, e foi o seletor:** meu teste da pista lia opacidade 1,00 em todo o curso. `sec.querySelectorAll("div")` devolve em ordem de documento, e o CARTÃO também tem dois filhos e também contém `.seta-rolagem` — eu estava medindo o cartão. Com `.seta-rolagem`.parentElement a curva aparece: 0 → 1 → 0. **Confirmar QUAL elemento está sendo lido antes de acreditar na leitura.**
- 2026-09-24 — **A FOTO DE PERFIL DA CONVERSA É A MARCA DA CLÍNICA**, a pedido ("logo da Suzuki na foto de perfil do whatsapp"). Era o glifo genérico do WhatsApp, que dizia qual é o aplicativo e nada sobre quem atende. É a logo COMPLETA (`brand.logoEscuro`), não um recorte do símbolo: a regra de 19/08 já foi paga uma vez, quando eu recortei os traços da linha de baixo e ele reprovou na hora ("a logo da Suzuki não tá completa"). Campo BRANCO e arte escura, porque a barra do contato é `#1f2c34` e a logo branca sobre ela some. `logoEscuro` é opcional no tipo, então numa variante sem a arte escura volta o glifo do WhatsApp: círculo branco vazio lê como imagem quebrada. Medido: logo de 46,5x20,5 num círculo de 62px no desktop e 27,9x12,3 num de 37px no celular, dentro dos limites nos dois.
- 2026-09-25 — **A COPY DA CHAMADA EM ROLAGEM TROCOU**, escolhida por ele entre QUATRO opções que foram renderizadas no site real e apresentadas por print (desktop e celular), não por texto solto. Ficou a "conte o seu caso": título `Conte o seu caso. A gente avalia.`, descrição sobre a avaliação presencial definir diagnóstico, plano e orçamento, e os dois balões falando de CASO em vez de horário. ⚠️ **A manchete `Agende a sua avaliação.` ficou intacta**, por pedido explícito. As outras três estão no log desta sessão; o método (patch no `clinica.ts` + print por opção, e restaurar o arquivo no fim) está em `scratchpad/copy-opcoes.mjs`.
- 2026-09-25 — **O rótulo do botão é `Agende sua consulta`**, pedido no meio da rodada e por cima da opção escolhida (ela trazia "Começar pelo WhatsApp"). ⚠️ É a ÚNICA peça do site que diz "consulta"; o resto do texto fala em "avaliação". Foi pedido dele, não descuido, e a diferença fica registrada aqui para não ser "uniformizada" numa próxima sessão.
- 2026-09-25 — ⚠️ **"NO CELULAR NÃO QUERO QUE FIQUEM DUAS LINHAS O BOTÃO", e `whitespace-nowrap` SOZINHO NÃO RESOLVE** — ele só troca quebra por transbordo. O que faz caber é a conta: em 320px o cartão dá **256px úteis**, e com `px-8`, ícone de 24px e rótulo a 18px a pílula pedia 275px. No celular ela passou a `px-5`, `gap-2.5`, ícone de 20px e rótulo a 16px, e fecha em **258px**; de `md` para cima tudo volta ao tamanho cheio. **Ao trocar o rótulo deste botão, remedir em 320px** — é lá que estoura, e 390px passa sem avisar.
- 2026-09-25 — ⚠️ **Item de flex MAIOR que o contêiner não centraliza: com `justify-center` ele transborda para a DIREITA.** Foi o estado intermediário desta rodada, a 320px: a pílula ficou com 20px de folga de um lado contra 4px do outro, ou seja fora de eixo, sem gerar overflow de página (o cartão é `overflow-hidden`) e sem quebrar linha. **Os dois testes de sempre PASSAM nesse defeito** — conferir a folga dos DOIS lados, não só se cabe.
- 2026-09-25 — ⚠️ **`getClientRects().length` num elemento de BLOCO devolve 1, não o número de linhas** — meu primeiro medidor de quebra do título dava "1L" para um `h2` de três linhas. O certo é `Range.selectNodeContents(el)` e contar os retângulos DO RANGE. É a mesma família do medidor errado de 13/08, que comparava largura de span com a do `h1`.
- 2026-09-25 — **A SEÇÃO DO RESPONSÁVEL VIROU A PRIMEIRA DEPOIS DO HERO**, a pedido com o print do hero na mão ("precisamos valorizar o Dalton, a seção dele precisa ser a primeira após essa que printei"). Era a SEXTA, entre a chamada e Diferenciais, desde 15/09. ⚠️ Isso muda o argumento da página pela TERCEIRA vez: até 13/08 ela abria pelos diferenciais, passou a abrir pelo trabalho feito (Casos) e agora abre por QUEM conduz o trabalho. O menu e a coluna "Clínica" do rodapé foram na mesma ordem: "Equipe" subiu para o segundo item da pílula e "Responsável técnico" para o primeiro da coluna. Ordem nova: portal → hero → **responsável** → casos → áreas → chamada → diferenciais → estrutura → depoimentos → faq → localização → rodapé.
- 2026-09-25 — ⚠️ **O defeito das duas faixas escuras encostadas voltou pela TERCEIRA vez**, e a medição pegou antes do print: hero e Bio são os dois blocos escuros sangrados, nenhum participa do ritmo de `--section-py`, e no lugar novo o vão entre as pinturas deu **0px** — dois cantos de raio 24px colados, com o fio claro entre eles lendo como risco no meio de um bloco só. Mesmo conserto de 13/08 e 15/09: `pt-6 md:pt-8` na Bio, o **dobro da goteira** lateral. Medido depois: 32px no desktop, 24px no celular. **Ao mover faixa sangrada, conferir quem passa a vir antes.**
- 2026-09-25 — Medido depois da troca de ordem, em 1440×900 e 390×844: ordem `portal → top → responsavel → casos → areas → chamada → diferenciais → estrutura → depoimentos → faq → localizacao`, **zero âncora morta** no menu e no rodapé, zero overflow lateral, e a folga da pílula em 1024 intacta (39px em `/casos`, 37px em 1280) — a contagem de itens não mudou, só a ordem. `tsc --noEmit` e `vite build` limpos.
- 2026-09-25 — **A METADE DO RESPONSÁVEL VIROU TEMPLATE EDITORIAL**, de um componente do 21st.dev que o usuário mandou ("preciso mudar esse layout, não estou gostando"): retrato de um lado e o bloco de informação SOBREPONDO a borda dele, nome gigante e fino com o sobrenome um grau mais cheio. **Duas peças do original ficaram fora a pedido dele**: o botão redondo de seta e o rótulo de cargo. A esteira do corpo clínico NÃO foi tocada, porque o template só cobria a metade dele.
- 2026-09-25 — ⚠️ **O rótulo de cargo saiu e NÃO virou outra coisa no mesmo lugar.** Quem carrega a identificação é o par nome + `CRO-PR 9112`, e o CRO está logo abaixo do nome, que é onde a CFO-196/2019 exige os dois juntos. O nome virou DOIS campos de conteúdo (`nomeLinha1`/`nomeLinha2`) porque onde quebrar é decisão de conteúdo: partir por código no último espaço erra em nome composto, e as variantes de Rogério e Décio vão precisar disso. `bio.nome` e `bio.eyebrow` saíram do tipo — o segundo já estava morto havia tempo.
- 2026-09-25 — **`framer-motion` do template NÃO entrou**, e é a quarta vez que essa recusa aparece (GSAP e `motion/react` em 19/08, `motion/react` de novo em 19/08). O projeto não tem dependência de animação, e o motivo é duro: essas libs escrevem a propriedade `transform`, o Tailwind v4 escreve `translate`/`scale`/`rotate` SEPARADAS, e misturar as duas famílias no mesmo elemento faz uma apagar a outra **sem erro nenhum**. A entrada é o `Reveal` que o site inteiro já usa.
- 2026-09-25 — ⚠️ **O PRIMEIRO RENDER ENEVOOU O RETRATO INTEIRO, e a causa é a inversão de fundo.** No template o texto é ESCURO sobre página clara, então cruzar a foto não custa contraste. Aqui é texto CLARO sobre bloco escuro, e a foto do Dalton é de estúdio com fundo CREME: texto claro cruzando creme simplesmente desaparece. O véu que resolve isso cobria o rosto na primeira tentativa (112px de 352, ou seja um terço da foto). Agora ele **desce apagando** por `mask-image`: forte nos 22% do topo, onde o nome cruza e ali é só fundo de estúdio, e ausente na altura do rosto. **Ao adaptar template de página clara para bloco escuro, o primeiro a conferir é onde o texto cruza imagem.**
- 2026-09-25 — ⚠️ **Tirar o botão de seta DESALINHOU o texto, e isso não é óbvio ao ler o template.** É o botão que empurra a bio para a direita no original; sem ele o texto caiu na borda do bloco e passou a correr **por cima do rosto**. A bio e as titulações ganharam `lg:pl-20`, que devolve o empurrão sem devolver o botão, e aí **só o NOME sobrepõe** — que é o gesto do original. **Ao remover peça de um template, procurar o que ela estava segurando no layout.**
- 2026-09-25 — **`.display-1-leve` criada: o par LEVE do primeiro degrau**, como `.display-3-leve` é o do terceiro, e pela mesma razão — peso e leading de um `.display-*` não se ajustam por utilitário do Tailwind, porque os degraus são declarados fora de `@layer` e vencem `@layer utilities` em silêncio. **Não é degrau novo de tamanho**: mesmo `clamp(2rem, 3.7vw, 3rem)` do `.display-1`, peso 200, com `.forte` aninhada em 400 para o sobrenome. A escala de 03/08 segue em cinco tamanhos.
- 2026-09-25 — Medido em 1440, 1024, 390 e 320: nome em 48px/peso 200 com o sobrenome em 400, retrato em **0% de recorte** (arquivo 300x400 numa caixa 3:4), sobreposição de 64px no desktop e ZERO no celular (ali as duas peças empilham e o véu nem renderiza), as quatro titulações presentes, nenhuma seta no DOM, zero overflow lateral. `tsc --noEmit` e `vite build` limpos.
...
- **`z-index` INTERNO de componente escapa e cobre a pílula de navegação.** Os cartões
  da galeria de casos usam `zIndex` inline até 100 e o header é `fixed z-50`; sem um
  contexto de empilhamento próprio, esse 100 compete no contexto RAIZ e a pilha passa
  por cima do menu. `relative` sozinho NÃO cria contexto — só cria com `z-index`
  declarado. `isolate` cria sempre. Corrigir no componente, não subindo o número do
  header: o do header é a camada da moldura da página, o do cartão é ordem interna.
...
- **O Google Fonts NÃO carrega no navegador deste ambiente** (`ERR_CONNECTION_RESET`;
  o `curl` alcança). Todo screenshot sai com a fonte de RESERVA do sistema, não com a
  Instrument Sans — não confundir isso com defeito de tipografia. Para conferir se uma
  fonte carregou, medir a largura do mesmo texto nela e na reserva; `document.fonts.check()`
  responde `true` para a reserva.
...
- 2026-08-20 — Medido: com a pilha cruzando a faixa da pílula (topo em 24px contra base em 98px), `elementFromPoint` no centro da pílula devolve elemento DO HEADER em 1440 e em 390 — antes devolvia o cartão. E o arraste continua funcionando (contador de 1/5 para 3/5 num gesto de 240px), ou seja o contexto novo não capturou o ponteiro.
- 2026-08-19 — ⚠️ **"Tem um baita espaço antes do hero" era DEFEITO MEU, e a causa é uma divisão.** O progresso da tela de entrada vinha de `-top / (altura - innerHeight)`, ou seja media só o trecho em que o palco está GRUDADO — então a opacidade chegava a ZERO com o palco ainda ocupando a tela inteira, e sobrava **uma tela cheia** de verde vazio rolando antes do hero. Medido antes: no fim do curso a marca em opacidade 0 e o topo do hero a 900px. Dividindo pela altura INTEIRA da seção, o apagamento termina junto com ela: as peças continuam na tela enquanto o palco desliza e o hero sobe por baixo, e em nenhum momento a tela fica vazia.
- 2026-08-19 — Junto: `TRILHO_MULT` de 0,75 para **0,4** (seção de 1575 para 1260px, o hero chega 315px antes) e os deslocamentos reduzidos de 0,42/0,38 para 0,22/0,20 — agora boa parte do curso é a SAÍDA do palco, e nela a rolagem da página já carrega tudo para cima de graça; somar o deslocamento antigo em cima disso jogaria as peças fora do quadro cedo demais, recriando a tela vazia por outro caminho.
- 2026-08-19 — ⚠️ **O piso desse número é o próprio palco.** Ele tem uma tela de altura e precisa sair de cena antes do hero entrar, então não existe ajuste que faça o hero aparecer em menos de ~1 tela de rolagem. O que se ajusta é o que aparece DURANTE essa saída — e é isso que estava errado, não a distância.
- 2026-08-19 — Medido depois, em 1440×900 e 390×844: seção em 1,4 tela, opacidade 1 no fim do trecho grudado (era 0), hero 220px visível já com a marca na tela, zero overflow, menu em 0 até 1200 e revelando em 1320 com limiar 1260. Página em 11,5 telas.
...
- 2026-08-19 — ⚠️ **O afastamento ALARGAVA a página em 997px** antes do recorte — os elementos que saem para os lados criam rolagem horizontal. `overflow-hidden` no PRÓPRIO palco resolve; num ANCESTRAL do `sticky` mataria a grudagem (defeito pago na Bio em 13/08). Medido depois: overflow 0 em todos os pontos do curso, em 1440 e em 390.
- 2026-08-19 — O limiar do menu virou **`PORTAL_VH + ABERTURA_NAV_VH`**: a arcada deixou de ser a primeira seção, então o ponto em que ela completa está 1,75 tela mais abaixo do que o componente dela sabe. Sem somar, o menu apareceria no meio da tela de entrada. Os dois valores são derivados e exportados por cada seção — nenhum número solto no Header. Medido: opacidade 0 até y=2760 e revela em 2800, com o limiar em 2772.
- 2026-08-19 — "Home" no menu passou a apontar para **`#portal`**. Era `#abertura` (a arcada), que agora é a SEGUNDA tela — clicar levaria para o meio da abertura, pulando a entrada. Terceira vez que esse href precisa mudar junto com a ordem das seções; ao mexer na ordem, conferir este e o da marca no Header.
- 2026-08-19 — ⚠️ **NONO e DÉCIMO falsos positivos de medição da sessão, um atrás do outro.** (a) O portal "não se movia" até 0,35 do curso no desktop — o `scrollY` do meu script nem estava sendo aplicado, e só apareceu ao imprimir `window.scrollY` junto da medida. (b) O menu "aparecia durante a tela de entrada" — meu seletor pegava o `<nav>` interno e depois um filho do `<header>`, quando quem carrega a opacidade é o **próprio `<header>`**. Nas duas vezes o código estava certo. **Antes de consertar, imprimir a variável de entrada da medição e confirmar QUAL elemento está sendo lido.**
...
- 2026-08-21 — ⚠️ **O usuário subiu a skill uma SEGUNDA vez, pelo GitHub web**, em três commits "Add files via upload": os sete `.md` soltos na RAIZ do repo mais um `10k-websites-skill.zip`. Conferido byte a byte: os sete são idênticos aos de `.claude/skills/10k-websites/`, e o zip tem os mesmos sete dentro. Os oito arquivos da raiz foram removidos e a cópia canônica é a de `.claude/skills/` — **é ela que o Claude Code registra como skill**; `SKILL.md` na raiz não é skill nenhuma, é um arquivo solto que o sync do Lovable carrega junto. Nada se perdeu, e o histórico dos uploads dele fica no git.

- 2026-08-19 — **O RETRATO DO DALTON ENTROU NO HERO**, a pedido: "vamos colocar o rosto do Dalton, ele é a principal cara da Suzuki, seria essencial ter a foto dele ao lado da logo no hero e embaixo uma assinatura manuscrita do nome completo dele, tipo do Ayrton Senna". O arquivo NÃO é geração nova — é o `hero/dalton-suzuki.webp` (500×482) que já estava no repo, foi a foto única do hero até 13/08 e voltou do arquivo morto. Entra com `.retrato-fundido`, porque é FOTO retangular e não recorte com alpha; sem a máscara ela desenha uma aresta reta no bloco escuro.
- 2026-08-19 — ⚠️ Ele disse "ao lado da logo", mas a logo vive na TELA DE ABERTURA e o hero não tem logo nenhuma. Perguntei, e ele escolheu **o hero, ao lado do texto** — não a abertura. Ou seja: não acrescentar logo ao hero para "cumprir" a frase; a marca aparece UMA vez na home, grande, na tela de entrada (`AberturaPortal`), e isso é decisão de 17/08.
- 2026-08-19 — **A CENTRALIZAÇÃO NO ESTILO APPLE DUROU UM DIA.** "Ao lado" é grade de duas colunas, que é o oposto de centralizar — então o hero voltou a ter retrato à esquerda e texto à direita. O que sobrou da referência: texto curto, muito respiro, duas ações lado a lado, e a figura com presença de verdade em vez de miniatura.
- 2026-08-19 — A grade abre em `lg` e NÃO em `md`, e é a headline que manda: a linha "complexidade, conduzida" mede 668px com `.display-1` no teto. Abrindo em 768px a coluna de texto ficaria com ~380px e a headline quebraria em cinco linhas. Abaixo de `lg` a composição empilha e volta a ser centrada. Medido em 1920/1440/1280/1024/390: **1 retângulo por linha em todas**, ou seja nenhuma quebrou.
- 2026-08-19 — ⚠️ **`hero.arcada` DEIXOU DE EXISTIR no tipo**: `hero.retrato` ocupou o lugar. O campo apontava para um quadro do vídeo da arcada 3D, e essa animação foi apagada do site no fim de 19/08 (está em `b4292fc`) — deixá-lo era manter um slot de imagem morto. Nas variantes de Rogério e Décio, `retrato` recebe a foto de cada um e `null` colapsa a coluna.
- 2026-08-19 — **A ASSINATURA É PROVISÓRIA, E ISSO ESTÁ DECLARADO EM TRÊS LUGARES** (o tipo, o `title` do elemento e o LEIA-ME de `public/fontes/`). A referência que ele deu é a do Ayrton Senna, que é caligrafia REAL digitalizada — para ser a do Dalton, precisa do traço dele. Ofereci três caminhos e ele escolheu **fonte manuscrita agora, como reservado**. O terceiro caminho — eu desenhar uma caligrafia — foi apresentado com a ressalva de que seria atribuir a um profissional real um traço que não é o dele, e ele não escolheu esse. **Não "resolver" isso desenhando.** Fechar é: ele assina em papel branco com caneta preta grossa, fotografa, eu vetorizo, `hero.assinatura.src` aponta para o SVG — e então a fonte, o @font-face e a classe saem do projeto.
...
- 2026-09-15 — **A FONTE DA ASSINATURA TROCOU: Caveat → Qwitcher Grypen**, a pedido, com uma IMAGEM de referência anexada ("uma assinatura estilo essa"). A Caveat é letra de MARCADOR — de mão, desligada, vertical — e não é assinatura. Renderizei SEIS famílias lado a lado com "Dalton Suzuki" sobre o petróleo antes de escolher (Qwitcher Grypen, Ms Madi, Kolker Brush, Yellowtail, Dancing Script, Caveat): a Qwitcher é a única com as três marcas da referência juntas — traço de largura constante, letras ligadas e inclinação forte. As duas mais próximas depois dela, se ele quiser trocar, são Ms Madi (mais vertical) e Kolker Brush (pincel irregular).
- 2026-09-15 — ⚠️ **Em cursiva ligada, a altura de x é muito menor que o corpo**, então trocar de família sem mexer no tamanho encolhe a peça pela metade a olho. O clamp foi de `(1.75rem, 3.2vw, 2.5rem)` para `(2rem, 4.4vw, 3.5rem)` e o peso de 600 para **700**, o mais cheio que a família tem — a 400 o traço monolinear fino desaparece sobre o petróleo. Medido: "Dalton Suzuki" fecha em 228px a 56px (1440), 183px a 45px (1024) e 130px a 32px (390).
- 2026-09-15 — **A MÁSCARA DO RETRATO SAIU E ENTROU SOMBRA** ("a foto do Dalton sem borda com sombra"). `.retrato-fundido` dissolvia as quatro bordas para a foto PERTENCER ao fundo; pedir sombra é pedir o contrário — que ela seja um objeto POUSADO sobre ele. As duas coisas se anulam: sombra de borda dissolvida não existe. ⚠️ Isso reintroduz "cartão com sombra própria", que o §5.2 lista como forma removida em 03/08 — voltou por pedido explícito e numa peça só, **não é licença para devolver sombra às outras seções**.
...
- 2026-09-15 — ⚠️ **"TEM UMA SOMBRA ENTRE A SESSÃO DE SCROLLAGEM E O HERO", com print, e não era sombra: era um BRILHO.** O `.ink-arc` tinha DOIS gradientes, e o segundo é um brilho branco a 9% ancorado em `50% -10%` — ou seja no TOPO do bloco. Como a tela de entrada logo acima é o MESMO `--ink`, o encontro das duas superfícies virava um degrau visível de ponta a ponta. Medido na coluna central em 1440: **23,69 de luminância acima da borda contra 26,80 logo abaixo**, voltando a 23,69 em ~80px. Três níveis em 255 bastam para ler como costura num campo chapado. O gradiente saiu; o arco DOURADO da base fica, porque nunca tocou borda nenhuma.
...
- 2026-09-15 — ⚠️ **"NO CELULAR O SCROLL ESTÁ TRAVANDO MUITO", e o meu primeiro palpite estava ERRADO.** Culpei a `mask-image` que eu tinha posto no pé do palco horas antes e fui medir antes de mexer: A/B em celular simulado com CPU 4× mais lenta, duas rodadas alternadas, deu **empate** (a rodada 1 favoreceu a máscara, a 2 o contrário). Não era ela. **Medir antes de trocar a solução** — é a mesma lição do `colorlevels` em 19/08.
- 2026-09-15 — O que a medição ACHOU, e virou conserto: (a) **o laço da tela de entrada rodava a página inteira** — um `getBoundingClientRect` e duas escritas de estilo por quadro ao longo de dez telas de rolagem, por uma seção que já passou. Agora ele sai fora quando a seção não está mais na janela, e só escreve quando o valor MUDA. (b) **As três esteiras andavam mesmo fora da tela**: laço infinito com faixa larga de imagens e `mask-image` por cima, e o compositor mantém essas camadas trabalhando a cada quadro. `useParadaForaDaTela` (IntersectionObserver com 200px de folga) as congela fora de vista. Medido: no topo da página, 3 de 3 paradas.
- 2026-09-15 — ⚠️ **O celular simulado NÃO reproduz o travamento dele** — mediana de 16,7ms (60fps) em todas as seções, antes e depois. Os dois consertos acima são reduções de trabalho reais e medidas, mas **a confirmação tem de vir do telefone dele**. Se continuar travando, o próximo suspeito medível é a `mask-image` das TRÊS esteiras (`.esteira-mask`), que é máscara em elemento largo com imagem dentro — o mesmo padrão que eu suspeitei no palco, agora multiplicado por três e permanente.
...
- 2026-09-18 — **TRÊS DIREÇÕES DE PALETA propostas, medidas e aplicadas no site REAL** (`docs/paletas.md` + `docs/paletas/*.css` + `scripts/comparar-paletas.mjs`). ⚠️ **Nenhuma está em vigor** — a publicada continua a verde + branco de 30/07. O pedido era explícito em não entregar três versões do mesmo caminho, então elas mudam de EIXO e não de matiz: **A (grafite + um sinal)** mantém o verde e o torna raro, tirando o dourado do sistema; **B (areia + bronze)** abandona o verde e assume a madeira mel das 12 fotos como ambiente; **C (noturno)** põe a página inteira no petróleo e promove o dourado a texto.
- 2026-09-18 — Aplicadas por injeção de `:root` no site rodando, e não em amostra de cor: quatro telas por paleta (hero, casos, áreas, bio) que cobrem os quatro contextos de cor do site — bloco escuro sangrado, página clara com foto, painel escuro dentro de seção clara, faixa escura com retratos. As comparações saem em contact sheet, as quatro variantes da MESMA tela lado a lado, que é o único formato em que dá para decidir.
- 2026-09-18 — **Os três estados do botão são renderizados no COMPONENTE REAL**, clonado da página: normal, hover e desabilitado. ⚠️ Duas correções aí, as duas achadas no primeiro render: (a) o hover depende do TOM do botão e eu tinha cravado `--ink-elevated` — o CTA do hero é `tone="light"`, então o hover dele é `--surface-raised`, e a primeira tira saiu com a pílula clara pintada de escuro; agora o script DESCOBRE o tom comparando o fundo computado com `--surface`. (b) A paleta atual não tem estado desabilitado, então a célula saía idêntica à normal e lia como bug do script — agora ela é rotulada "não existe hoje".
- 2026-09-18 — ⚠️ **ACHADO NO SITE ATUAL, não nas propostas: o contorno das setas da galeria REPROVA em contraste — 1,63:1 contra os 3:1 exigidos.** Aquelas setas são botão sem fundo, então o fio (`--border-strong`) é a única fronteira do controle e vale a regra de não-texto da WCAG 1.4.11. As três paletas novas corrigem (3,2 a 3,4); a atual corrige subindo o alfa de 0,24 para ~0,45. **Não confundir com os DIVISORES** (`--border`, `--ink-border`, 1,3 a 1,5): decoração é isenta, e um divisor a 3:1 seria um traço preto atravessando a página.
- 2026-09-18 — Segundo achado do mesmo tipo: **o projeto não tem estado desabilitado em lugar nenhum**. Nunca fez falta porque nenhum botão do site está desabilitado, mas o sistema de cor estava incompleto. As três propostas trazem `--disabled-surface`, `--disabled-foreground` e `--disabled-border`, e os três passam 4,5:1 — a WCAG isenta controle inativo, mas botão desabilitado ilegível é defeito de produto de todo jeito.
- 2026-09-18 — ⚠️ **O CUSTO DA C FOI MEDIDO, não estimado, e confirma a decisão de 30/07** ("trocar clínica = trocar token vale para mudar de matiz, não para inverter claro/escuro"). Duas quebras concretas: o `PillButton` sem `tone` — só existe um, o "Ver a clínica inteira" em `Estrutura.tsx` — fica `--ink` sobre `--background` a **1,07:1**, ou seja botão invisível; e o `brand.logoEscuro` do cartão de avaliações cai sobre superfície de luminância **0,006**, some. São duas linhas de componente. **A e B não custam nenhuma.**
- 2026-09-18 — Medição de contraste por **rasterização**, e é a regra de 21/08 aplicada de propósito: `getComputedStyle` devolve a string `oklch(...)` e lê-la como RGB dá cor inventada. Cor com alfa é composta sobre o fundo REAL antes de medir, senão o canvas transparente lê quase-preto. O script também confere `--background` SERVIDO antes de acreditar em qualquer leitura — a armadilha do CSS velho de 15/09.
- 2026-09-18 — ⚠️ Ressalva que vale para toda esta rodada: **os screenshots não mostram a Instrument Sans**, porque o navegador deste ambiente não alcança o Google Fonts. Não afeta cor nem contraste, afeta o "ar" das telas. E **nenhuma das três foi otimizada para as fotos do acervo** — elas são madeira mel + granito + parede branca, o que favorece a B por construção; é vantagem real dela, não empate.
